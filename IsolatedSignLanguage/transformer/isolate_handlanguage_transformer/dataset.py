...
def read_dataset(test_number, val_number, input_dir: Path = config.read_dataset_dir) -> Tuple[List, List, List, int]:
    ...
    train_hdf5files = [fin for fin in hdf5_files if test_number not in fin.name]
    val_hdf5files = [fin for fin in hdf5_files if val_number in fin.name]
    test_hdf5files = [fin for fin in hdf5_files if config.test_number in fin.name]
