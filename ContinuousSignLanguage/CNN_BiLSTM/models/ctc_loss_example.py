...
def detailed_ctc_loss_calculation():
    ...
    for i in range(C):
        print(f"     クラス{i}: 確率={probs[i]:.4f}, ログ確率={log_probs_sample[i]:.4f}")
...
def step_by_step_example():
    ...
    # 手動で設定したロジット値
    logits = torch.tensor([[[2.0, 1.0, 0.5],   # t=0: blank有利
                           [0.5, 2.5, 1.0],   # t=1: class1有利
                           [1.0, 0.5, 2.0]]], # t=2: class2有利
                         dtype=torch.float32)
    ...
    for t in range(T):
        ...
        for c in range(C):
            ...
            print(f"    {class_name}: {probs[t, 0, c]:.4f} (log: {log_probs[t, 0, c]:.4f})")
